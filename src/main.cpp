...
void display_mode_A(); //A=新要素１
void display_mode_B(); //B=新要素２
void display_field();//コート図面
...
uint8_t coord_x,coord_y;//ロボットの座標
...
void loop() {
      ...
      if (switch_right_1) {
            if (set_mode == 0 && display_mode < 11) display_mode += 1;
            set_value = 1;
      }
      ...
      if (set_mode == 0) {
            ...
            if (display_mode < 11) oled.fillTriangle(117, 53, 128, 58, 117, 63, WHITE);
      }
